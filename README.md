...
GoWind ERP 当前处于**基座建设阶段**。组织权限、认证、审计、文件存储、字典、站内消息、任务调度、多语言等通用基座能力已实现，并已提供管理后台与移动端入口。ERP 业务模块（库存 / WMS、采购 / SRM、财务 / AP）及对应的移动端功能仍在规划与开发中，**尚未纳入当前代码库**。下文“核心功能”中标注为“规划中”的条目即属此类。
...
### ERP 业务模块（规划中，当前版本尚未实现）

| 模块          | 说明                              |
|:------------|:----------------------------------|
| 库存 / WMS    | 仓库、库位、库存原子操作与状态机（模块 1，开发中） |
| 采购 / SRM    | 供应商与采购流程（模块 2，规划中）         |
| 财务 / AP     | 应付与财务核算（模块 3，规划中）         |
| 移动端业务功能    | WMS 扫码、审批中心、经营看板（规划中，待后端就绪） |
