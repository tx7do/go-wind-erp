...
func (_m *PurchaseOrder) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case purchaseorder.FieldUpdatedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
		...
		case purchaseorder.FieldDeletedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
		...
		case purchaseorder.FieldPoNumber:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.PoNumber = new(string)
				*_m.PoNumber = value.String
			}
		case purchaseorder.FieldSupplierCode:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case purchaseorder.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Status = new(purchaseorder.Status)
