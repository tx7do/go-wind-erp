  ...
  async onConfirm() {
    ...
    try {
      await createReturnMutation.mutateAsync({
        purchaseOrderId: data.value.poId,
        items: [
          {
            purchaseOrderItemId: data.value.poItemId,
      ...
      notification.success({
        message: $t('page.stockPicking.return.purchaseSuccess'),
      });
    } catch {
      notification.error({
...
<template>
  <Modal :title="$t('page.stockPicking.return.purchaseTitle')">
    <BaseForm />
  </Modal>
</template>
