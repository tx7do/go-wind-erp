...
import {
  ...
  makeUpdateMask,
  PaginationQuery,
  type productservicev1_Product as Product,
} from '#/api';
...
const gridOptions: VxeGridProps<Product> = {
  ...
  columns: [
    ...
    { title: $t('page.product.code'), field: 'code' },
    { title: $t('page.product.name'), field: 'name' },
    { title: $t('page.product.spec'), field: 'spec' },
    { title: $t('page.product.unit'), field: 'unit' },
    {
      title: $t('page.product.enable'),
      field: 'enable',
...
async function handleDelete(row: any) {
  try {
    await apiClient.productService.Delete({ id: row.id });
...
async function handleEnableChanged(row: any, checked: boolean) {
  ...
  try {
    await apiClient.productService.Update({
      id: row.id,
...
<template>
  <Page auto-content-height>
    <Grid :table-title="$t('menu.wms.product')">
      <template #toolbar-tools>
        <a-button class="mr-2" type="primary" @click="handleCreate">
          {{ $t('page.product.button.create') }}
        </a-button>
      </template>
        ...
        <a-popconfirm
          ...
          :title="
            $t('ui.text.do_you_want_delete', {
              moduleName: $t('page.product.moduleName'),
            })
          "
