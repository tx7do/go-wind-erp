...
export async function fetchAllDictEntries() {
  ...
  const all = new PaginationQuery({ paging: { page: 1, pageSize: 1000 } });
  const types = await apiClient.dictTypeService.List(all.toRawParams());
  const result = await apiClient.dictEntryService.List(all.toRawParams());
